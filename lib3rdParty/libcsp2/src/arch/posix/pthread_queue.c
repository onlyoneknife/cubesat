...
int pthread_queue_enqueue(pthread_queue_t * queue, void * value,
		uint32_t timeout) {
	...
	memcpy(queue->buffer + (queue->in * queue->item_size), value,
			queue->item_size);
	queue->items++;
...
int pthread_queue_dequeue(pthread_queue_t * queue, void * buf, uint32_t timeout) {
	...
	pthread_mutex_lock(&(queue->mutex));
	while (queue->items == 0) {
		ret = pthread_cond_timedwait(&(queue->cond_empty), &(queue->mutex),
				&ts);
		if (ret != 0) {
			pthread_mutex_unlock(&(queue->mutex));
	...
	memcpy(buf, queue->buffer + (queue->out * queue->item_size),
			queue->item_size);
	queue->items--;
