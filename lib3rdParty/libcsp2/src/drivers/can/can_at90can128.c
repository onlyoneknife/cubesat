...
void can_configure_mobs(void) {
	...
	for (mob = 0; mob < CAN_MOBS; mob++) {
		...
		if (mob >= CAN_TX_MOBS) {
			...
			/* Accept extended frames */CAN_CLEAR_IDEMSK();
			CAN_CONFIG_RX()
			;
		}
...
int can_send(can_id_t id, uint8_t data[], uint8_t dlc,
	...
	/* Disable CAN interrupt while looping MOBs */CAN_CLEAR_INTERRUPT();
	...
	/* Enable CAN interrupt */CAN_SET_INTERRUPT();
	...
	/* Set DLC */CAN_CLEAR_DLC();
	CAN_SET_DLC(dlc);

	/* Start TX */CAN_CONFIG_TX()
	;
...
ISR( CANIT_vect) {
	...
	while (CAN_HPMOB() != 0xF) {
		...
		if (CANSTMOB & ERR_MOB_MSK) {
			...
			/* Clear status */CAN_CLEAR_STATUS_MOB()
			;
