...
typedef struct {
	...
	typedef enum {
		CAN_ERROR = 0, CAN_NO_ERROR = 1,
	} can_error_t;

	/** TX Callback function prototype */
	typedef int (*can_tx_callback_t)(can_id_t id, can_error_t error,
	...
	int can_init(uint32_t id, uint32_t mask, can_tx_callback_t txcb,
			can_rx_callback_t rxcb, struct csp_can_config *conf);
