...
static s16 _waitUntilSleep(HANDLE si114x_handle)
{
    ...
    while (1)
    {
        data = Si114xReadFromRegister(si114x_handle, REG_CHIP_STAT);
        if (data == 1) break;
...
s16 Si114xReset(HANDLE si114x_handle)
{
    ...
    retval+=Si114xWriteToRegister(si114x_handle, REG_ALS_RATE, 0x0); //for uv part
...
s16 Si114xParamRead(HANDLE si114x_handle, u8 address)
{
    ...
    data = Si114xReadFromRegister(si114x_handle, REG_PARAM_RD);
    return data;
}
...
{	u8 data;
	...
	/*  After a RESET, if the Si114x receives a command (including NOP) before the Si114x has gone to sleep, the chip hangs.
		This first while loop avoids this.  The reading of the REG_CHIPSTAT does not disturb the internal MCU.
	 */

	data = 0; //initialize data so that we guarantee to enter the loop
	while (data != 0x01)
	{	data = Si114xReadFromRegister( si114x_handle, REG_CHIP_STAT);
	}
	...
	while (1)
    {
        ...
        while (1) 
        {	data = Si114xReadFromRegister(si114x_handle, REG_RESPONSE);
        	if(data == 0)
                break;
        	else
        	{
...
#define FLT_TO_FX20(x)       ((s32)((x*1048576)+.5))
...
//
static s16 find_cal_index( u8 *buffer )
{
    s16 index;
    u8  size;
...
static u32 vispd_correction(u8 *buffer)
{
    ...
    struct operand_t op;
    u32              result;
    s16              index = find_cal_index( buffer );
