...
static void CMU_FlashWaitStateControl(uint32_t hfcoreclk) {
	...
	/* BUS_RegMaskedWrite cannot be used here as it would temporarely set the
	 mode field to WS0 */MSC ->READCTRL = (MSC ->READCTRL
			& ~_MSC_READCTRL_MODE_MASK) | mode;
 ...
 ******************************************************************************/__STATIC_INLINE uint32_t CMU_DivToLog2(
 ...
 * @param[in] log2
 * @return Dividend
 ******************************************************************************/__STATIC_INLINE uint32_t CMU_Log2ToDiv(
		uint32_t log2) {
	return 1 << log2;
}
	...
	case CMU_LFAPRESC0_REG:
		...
		case cmuClock_RTC:
			ret = (CMU_ClkDiv_TypeDef) (((CMU ->LFAPRESC0
					& _CMU_LFAPRESC0_RTC_MASK) >> _CMU_LFAPRESC0_RTC_SHIFT));
			ret = CMU_Log2ToDiv(ret);
			break;
...
CMU_Select_TypeDef CMU_ClockSelectGet(CMU_Clock_TypeDef clock) {
	...
	statusClkSelMask = (CMU_STATUS_HFRCOSEL | CMU_STATUS_HFXOSEL
			| CMU_STATUS_LFRCOSEL |
...
void CMU_ClockSelectSet(CMU_Clock_TypeDef clock, CMU_Select_TypeDef ref) {
	...
	case CMU_LFBCLKSEL_REG:
		...
		case cmuSelect_CORELEDIV2:
			...
			BITBAND_Peripheral(&(CMU->HFCORECLKEN0), _CMU_HFCORECLKEN0_LE_SHIFT,
					1);
...
void CMU_OscillatorTuningSet(CMU_Osc_TypeDef osc, uint32_t val) {
	...
	case cmuOsc_AUXHFRCO:
		...
		CMU ->AUXHFRCOCTRL = (CMU ->AUXHFRCOCTRL
				& ~(_CMU_AUXHFRCOCTRL_TUNING_MASK)) | val;
		break;
