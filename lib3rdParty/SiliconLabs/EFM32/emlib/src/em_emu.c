...
static void EMU_Restore(void) {
	...
	oscEnCmd |= (
			(cmuStatus & CMU_STATUS_AUXHFRCOENS)? CMU_OSCENCMD_AUXHFRCOEN : 0);
...
void EMU_EnterEM4(void) {
	...
	em4seq2 = (EMU ->CTRL & ~_EMU_CTRL_EM4CTRL_MASK)
			| (2 << _EMU_CTRL_EM4CTRL_SHIFT);
	em4seq3 = (EMU ->CTRL & ~_EMU_CTRL_EM4CTRL_MASK)
...
void EMU_BUPDInit(EMU_BUPDInit_TypeDef *bupdInit) {
	...
	BITBAND_Peripheral(&(RMU ->CTRL), _RMU_CTRL_BURSTEN_SHIFT,
			!bupdInit->enable);
}
