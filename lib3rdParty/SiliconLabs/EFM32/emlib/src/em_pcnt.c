 ...
 * @return
 *   Instance number.
 ******************************************************************************/__STATIC_INLINE unsigned int PCNT_Map(
		PCNT_TypeDef *pcnt) {
 ...
 ******************************************************************************/
void PCNT_PRSInputEnable(PCNT_TypeDef *pcnt, PCNT_PRSInput_TypeDef prsInput,
		bool enable) {
	EFM_ASSERT(PCNT_REF_VALID(pcnt));
	...
	case pcntPRSInputS0: {
		BITBAND_Peripheral(&(pcnt->INPUT), _PCNT_INPUT_S0PRSEN_SHIFT,
				(uint32_t )enable);
	}
		break;
	...
	case pcntPRSInputS1: {
		BITBAND_Peripheral(&(pcnt->INPUT), _PCNT_INPUT_S1PRSEN_SHIFT,
				(uint32_t )enable);
	}
		break;
...
void PCNT_Init(PCNT_TypeDef *pcnt, const PCNT_Init_TypeDef *init) {
	...
	tmp = pcnt->INPUT
			& ~(_PCNT_INPUT_S0PRSSEL_MASK | _PCNT_INPUT_S1PRSSEL_MASK);
