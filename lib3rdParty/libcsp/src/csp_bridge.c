...
CSP_DEFINE_TASK(csp_bridge) {
	...
	while (1) {
		...
		csp_log_packet(
				"Input: Src %u, Dst %u, Dport %u, Sport %u, Pri %u, Flags 0x%02X, Size %"PRIu16,
...
int csp_bridge_start(unsigned int task_stack_size, unsigned int task_priority,
	...
	int ret = csp_thread_create(csp_bridge, (signed char *) "BRIDGE",
			task_stack_size, NULL, task_priority, &handle);
