	...
	if (MICROSD_SendCmd(CMD0, 0) == 1) { /* Enter Idle state */
		...
		if (MICROSD_SendCmd(CMD8, 0x1AA) == 1) { /* SDv2? */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) { /* The card can work at vdd range of 2.7-3.6V */
				while (!MICROSD_TimeOutElapsed()
						&& MICROSD_SendCmd(ACMD41, 0x40000000))
					; /* Wait for leaving idle state (ACMD41 with HCS bit) */
				if (!MICROSD_TimeOutElapsed()
						&& MICROSD_SendCmd(CMD58, 0) == 0) { /* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++)
						ocr[n] = MICROSD_XferSpi(0xff);
			...
			while (!MICROSD_TimeOutElapsed() && MICROSD_SendCmd(cmd, 0))
