 ...
 ******************************************************************************/
uint32_t
I2C_BusFreqGet(I2C_TypeDef *i2c)
{
  uint32_t hfperclk;
  uint32_t n;
 ...
 ******************************************************************************/
void
I2C_BusFreqSet(I2C_TypeDef *i2c, uint32_t refFreq, uint32_t freq,
 ...
 ******************************************************************************/
void
I2C_Enable(I2C_TypeDef *i2c, bool enable)
{
 ...
 ******************************************************************************/
void
I2C_Init(I2C_TypeDef *i2c, const I2C_Init_TypeDef *init)
{
EFM_ASSERT(I2C_REF_VALID(i2c));
...
BITBAND_Peripheral(&(i2c->CTRL), _I2C_CTRL_SLAVE_SHIFT, init->master ? 0 : 1);

I2C_BusFreqSet(i2c, init->refFreq, init->freq, init->clhr);

BITBAND_Peripheral(&(i2c->CTRL), _I2C_CTRL_EN_SHIFT,
...
for (;;)
  {
    ...
    case i2cStateDataSend:
      ...
      /* Send byte */
      i2c->TXDATA =
          (uint32_t) (seq->buf[transfer->bufIndx].data[transfer->offset++]);
...
i2c->IEN = I2C_IF_NACK | I2C_IF_ACK | I2C_IF_MSTOP | I2C_IF_RXDATAV
    | I2C_IF_ERRORS;
