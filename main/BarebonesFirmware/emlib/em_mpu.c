 ...
 ******************************************************************************/
void
MPU_ConfigureRegion(const MPU_RegionInit_TypeDef *init)
{
  EFM_ASSERT(
      init->regionNo < ((MPU->TYPE & MPU_TYPE_DREGION_Msk) >> MPU_TYPE_DREGION_Pos));
  ...
  if (init->regionEnable)
    {
      ...
      MPU ->RASR = ((init->disableExec ? 1 : 0) << MPU_RASR_XN_Pos)
          ...
          | (init->srd << MPU_RASR_SRD_Pos) | (init->size << MPU_RASR_SIZE_Pos)
