...
int main(void)
{
  ...
  /******** Read RTC register ***************************/
  uint8_t read = 0x01;  // Read
  uint8_t sub_address = 0x01;
  uint8_t reg_address = 0x0E; // Timer_control

  uint8_t command = ((read << 7) | (sub_address << 4)) | reg_address;

  uint8_t response = USART_SpiTransfer(USART1, command);
